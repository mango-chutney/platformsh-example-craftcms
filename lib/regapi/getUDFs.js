...
var _default = ({
  ...
  const params = (0, _paramify.default)(args);
  return fetch(`${baseURL}/UserDefinedFields?${params}`, {
    method: 'GET',
    headers: _objectSpread({
      Accept: 'application/json'
    }, headers)
  }).then(_handleFetchErrors.default).then(response => response.json()).then(response => Array.isArray((0, _first.default)(response)) ? (0, _first.default)(response) : response);
};
